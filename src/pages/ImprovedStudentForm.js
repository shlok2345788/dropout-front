...
const ImprovedStudentForm = () => {
  ...
  // Sample subjects for different streams/degrees
  const sampleSubjects = {
    'B.Tech': ['Mathematics', 'Physics', 'Chemistry', 'Computer Science', 'Electronics', 'Mechanics'],
    'B.Sc': ['Mathematics', 'Physics', 'Chemistry', 'Biology', 'English', 'Computer Applications'],
    'B.Com': ['Accountancy', 'Business Studies', 'Economics', 'Mathematics', 'English', 'Computer Applications'],
    'B.A.': ['History', 'Political Science', 'Economics', 'English', 'Sociology', 'Psychology'],
    'M.Tech': ['Advanced Mathematics', 'Advanced Physics', 'Computer Science', 'Electronics', 'Project Management'],
    'M.Sc': ['Advanced Mathematics', 'Advanced Physics', 'Advanced Chemistry', 'Research Methodology', 'Data Analysis'],
    'M.Com': ['Advanced Accountancy', 'Corporate Finance', 'Taxation', 'Business Analytics', 'Economics'],
    'M.A.': ['Advanced History', 'Political Theory', 'Economics', 'Literature', 'Research Methods'],
    'default': ['Mathematics', 'Science', 'English', 'Social Studies', 'Computer Applications']
  };
    ...
    setFormData(prev => {
      ...
      if (name === 'current_level') {
        if (value !== 'After 12th') {
          newData.degree_type = '';
          newData.degree_year = '';
          newData.degree_semester = '';
          newData.current_cgpa = '';
          newData.subjects = [];
          newData.subject_details = {};
        }
      }
      
      // Reset semester and CGPA when degree type changes
      if (name === 'degree_type') {
        newData.degree_semester = '';
        newData.current_cgpa = '';
        newData.subjects = [];
  ...
  const handleSubjectDetailChange = (subject, field, value) => {
    setFormData(prev => ({
      ...prev,
      subject_details: {
        ...prev.subject_details,
        [subject]: {
          ...prev.subject_details[subject],
          [field]: value
        }
      }
    }));
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      ...
      case 1: // Academic Background
        ...
        if (!formData.tenth_school.trim()) {
          setError('10th School name is required');
          return false;
        }
      ...
      case 4: // Study Habits & Lifestyle
        ...
        if (!formData.study_environment) {
          setError('Study environment description is required');
          return false;
        }
        break;
        
      case 5: // Goals & Aspirations
        ...
        if (!formData.expected_career_income) {
          setError('Expected career income is required');
          return false;
        }
        break;
        
      case 6: // Exam Preparation
        ...
        if (!formData.weak_subjects.trim()) {
          setError('Please mention your weak subjects');
          return false;
        }
        if (!formData.strong_subjects.trim()) {
          setError('Please mention your strong subjects');
          return false;
        }
        
        // Degree information validation for students who completed 12th
        if (formData.current_level === 'After 12th') {
          if (!formData.degree_type) {
            setError('Degree type is required');
            return false;
          }
          ...
          if (!formData.degree_semester) {
            setError('Semester is required');
            return false;
          }
          
          // For first semester, don't require CGPA but require subjects
          if (formData.degree_semester === '1st Semester') {
            if (!formData.subjects || formData.subjects.length === 0) {
              setError('Please select your subjects for the first semester');
              return false;
            }
            
            // Validate subject details for first semester
            for (const subject of formData.subjects) {
              if (!formData.subject_details[subject] || 
                  !formData.subject_details[subject].interest_level ||
                  !formData.subject_details[subject].difficulty_level) {
                setError(`Please provide details for subject: ${subject}`);
                return false;
              }
            }
          } else {
            // For other semesters, require CGPA
            if (!formData.current_cgpa) {
              setError('Current CGPA/Percentage is required for semesters beyond the first');
              return false;
            }
          }
        }
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>
                12th Standard Academic Performance
              </Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                name="twelfth_school"
                label="12th School Name (if applicable)"
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                label="12th Standard Percentage (if applicable)"
                type="number"
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                label="Describe Your Study Environment"
                value={formData.study_environment}
                onChange={handleChange}
                variant="outlined"
                helperText="Quiet room, library, with family members, etc."
              />
            </Grid>
            <Grid item xs={12}>
              <Grid container spacing={2}>
      ...
      case 5: // Goals & Aspirations
        return (
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth variant="outlined" required>
                <InputLabel>Expected Career Income</InputLabel>
                <Select
                  name="expected_career_income"
                  value={formData.expected_career_income}
                  onChange={handleChange}
                  label="Expected Career Income"
                >
                  <MenuItem value="Below 2L">Below ₹2 Lakhs/year</MenuItem>
                  <MenuItem value="2-5L">₹2-5 Lakhs/year</MenuItem>
                  <MenuItem value="5-10L">₹5-10 Lakhs/year</MenuItem>
                  <MenuItem value="10-20L">₹10-20 Lakhs/year</MenuItem>
                  <MenuItem value="Above 20L">Above ₹20 Lakhs/year</MenuItem>
          ...
          <Grid container spacing={2}>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} sm={4}>
                        <TextField
                          ...
                          label="Semester"
                          value={formData.degree_semester}
                          onChange={handleChange}
                          variant="outlined"
                          placeholder="1st, 2nd, 3rd Semester"
                        />
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <TextField
                      ...
                      <Grid item xs={12} sm={8}>
                        <Autocomplete
                          multiple
                          id="subjects"
                          options={sampleSubjects[formData.degree_type] || sampleSubjects['default']}
                      ...
                      {formData.subjects.map((subject) => (
                        <Grid item xs={12} sm={6} key={subject}>
                          <Box display="flex" alignItems="center">
                            <Chip label={subject} sx={{ mr: 1 }} />
                            <TextField
                              label="Interest Level"
                              variant="outlined"
                              value={formData.subject_details[subject]?.interest_level || ''}
                              onChange={(e) => handleSubjectDetailChange(subject, 'interest_level', e.target.value)}
                              sx={{ mr: 1 }}
                            ...
                            <TextField
                              ...
                              variant="outlined"
                              value={formData.subject_details[subject]?.difficulty_level || ''}
                              onChange={(e) => handleSubjectDetailChange(subject, 'difficulty_level', e.target.value)}
                              placeholder="High, Medium, Low"
                            />
                          </Box>
                        </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={handleChange}
                variant="outlined"
                placeholder="List subjects you find difficult"
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={handleChange}
                variant="outlined"
                placeholder="List subjects you excel in"
              />
            </Grid>
          </Grid>
        );
...
export default ImprovedStudentForm;